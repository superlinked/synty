...
enum Cmd {
    ...
    Mcp {
        ...
        /// Athena workgroup for bounded raw-event trace tools. When omitted,
        /// trace tools use the local projection.
        #[arg(long)]
        athena_workgroup: Option<String>,
        /// Glue database containing the raw event table.
        #[arg(long, default_value = "synty")]
        athena_database: String,
        /// Glue table whose single `line` column contains event envelopes.
        #[arg(long, default_value = "raw_events")]
        athena_table: String,
    },
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::Mcp {
            ...
            let athena = athena_workgroup
                .or_else(|| std::env::var("SYNTY_ATHENA_WORKGROUP").ok())
                .filter(|workgroup| !workgroup.is_empty())
                .map(|workgroup| mcp::AthenaTraceOptions {
                    workgroup,
                    database: athena_database,
                    table: athena_table,
                });
            if let Some(bucket) = &bucket {
                sync::pull_read_model_for_mcp(bucket, athena.is_none());
            }
            if http {
                let token = token
                ...
                mcp::run(model_id(), role, scope, redaction, bucket, athena)?
            }
        }
        Cmd::Summarize {
            sessions,
    ...
    fn mcp_parses_http_role_scope_and_redaction() {
        ...
        assert!(matches!(
            ...
            Cmd::Mcp {
                ...
                scope: Some(s),
                redaction: Some(r),
                athena_workgroup: Some(workgroup),
                athena_database,
                athena_table,
                allowed_origins,
                ..
                ...
                && s == "scope.json"
                && r == "mcp_safe"
                && workgroup == "synty-mcp-readonly"
                && athena_database == "synty"
                && athena_table == "raw_events"
