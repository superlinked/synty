...
mod tests {
    ...
    fn app() -> App {
        ...
        let work = vec![
            Unit { kind: Kind::Session, when: "2026-05-31".into(), repo: "sie".into(), title: "add OCR adapter".into(), outcome: "1 files".into(), summary: Some("Added an OCR adapter to the sie pipeline.".into()), topic: Some(0), rank: i64::MAX, struggle: 0.6, author: String::new(), doc_id: None, session_id: Some("S1".into()) },
            Unit { kind: Kind::Pr, when: "2026-05-31".into(), repo: "sie-web".into(), title: "sie-web#7 fix docs search".into(), outcome: "OPEN".into(), summary: None, topic: Some(0), rank: i64::MAX, struggle: 0.0, author: "alice".into(), doc_id: Some(0), session_id: None },
        ];
    ...
    fn clone_unit(u: &Unit) -> Unit {
        Unit { kind: u.kind, when: u.when.clone(), repo: u.repo.clone(), title: u.title.clone(), outcome: u.outcome.clone(), summary: u.summary.clone(), topic: u.topic, rank: u.rank, struggle: u.struggle, author: u.author.clone(), doc_id: u.doc_id, session_id: u.session_id.clone() }
    }
    ...
    fn app2() -> App {
        let mut a = app();
        let infra = Unit { kind: Kind::Pr, when: "2026-05-30".into(), repo: "infra".into(), title: "infra#3 terraform runner".into(), outcome: "MERGED".into(), summary: Some("Terraform runner IAM work.".into()), topic: Some(1), rank: i64::MAX, struggle: 0.0, author: "bob".into(), doc_id: None, session_id: None };
